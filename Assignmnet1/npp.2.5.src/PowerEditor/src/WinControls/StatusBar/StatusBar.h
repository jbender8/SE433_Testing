...
{
public :
	StatusBar() : Window(), _hloc(NULL), _lpParts(NULL), _partWidthArray(NULL) {};
	virtual ~StatusBar(){
        if (_hloc) 
        {
            ::LocalUnlock(_hloc);
    ...
    bool setText(const char *str, int whichPart) const {
        if (whichPart >= _nbParts) 
            return false;
		//bool b = ::SendMessage(_hSelf, SB_SETTEXT, whichPart, (LPARAM)str);
		return ::SendMessage(_hSelf, SB_SETTEXT, whichPart, (LPARAM)str);; 
    };
