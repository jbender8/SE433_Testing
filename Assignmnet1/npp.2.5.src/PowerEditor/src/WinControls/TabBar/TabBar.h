...
public:
	...
	static bool isOwnerDrawTab() {return (_drawInactiveTab || _drawTopBar);};
    ...
    bool setMultiLine() {
        if (!_isTraditional)
            return false;
        int style = (::GetClassLong(_hSelf, GCL_STYLE)) | TCS_MULTILINE;
        return ::SetClassLong(_hSelf, GCL_STYLE, style);
    };
    */
protected:
